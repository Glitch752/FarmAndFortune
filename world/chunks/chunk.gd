...
func generate_ground_polygon():
    ...
    for y in MapSingleton.CHUNK_SIZE:
        for x in MapSingleton.CHUNK_SIZE:
            ...
            var base_offset = Vector2(x, y) * tile_size
...
func _construct_jagged_poly(template: PackedVector2Array, offset: Vector2) -> PackedVector2Array:
    ...
    for i in range(count):
        ...
        var p1_world = (p1_local * tile_size) + offset
        var p2_world = (p2_local * tile_size) + offset
